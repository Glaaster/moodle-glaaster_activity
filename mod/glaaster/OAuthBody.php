 ...
 *
 * This function checks the request headers for an OAuth Authorization header.
 *
...
function get_oauth_key_from_headers($typeid = null, $scopes = null) {
    ...
    if (isset($requestheaders['Authorization'])) {
        if (substr($requestheaders['Authorization'], 0, 6) == "OAuth ") {
            ...
            $token = $DB->get_record('glaaster_access_tokens', $conditions);
            if ($token) {
                // Log token access.
                $DB->set_field('glaaster_access_tokens', 'lastaccess', $now, ['id' => $token->id]);
...
/**
 * Handle an OAuth request body post.
 *
