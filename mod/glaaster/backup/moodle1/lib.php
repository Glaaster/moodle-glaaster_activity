...
/**
 *  1.9 to 2.0 backup format converter for the LTI module.
 */
class moodle1_mod_glaaster_handler extends moodle1_mod_handler {
     ...
     *
     * The method returns list of {@see convert_path} instances.
    ...
    public function get_paths() {
        return [
            new convert_path(
                'basiclti',
                '/MOODLE_BACKUP/COURSE/MODULES/MOD/LTI'
            ),
        ];
    }
     ...
     * data available
     *
     * @param array $data The parsed data from the backup XML.
     */
    public function process_basiclti($data) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_glaaster');
        ...
        $this->xmlwriter->begin_tag('glaaster', ['id' => $instanceid]);

        $ignorefields = ['id', 'modtype'];
        if (!$DB->record_exists('glaaster_types', ['id' => $data['typeid']])) {
            $ntypeid = false;
            $toolurls = $DB->get_records_select(
                '{glaaster_types_config}',
    ...
    public function on_basiclti_end() {
        ...
        $this->xmlwriter->end_tag('glaaster');
