...
class restore_lti_activity_structure_step extends restore_activity_structure_step {
    ...
    /**
     * Define the structure of the lti activity.
     *
     * @return restore_path_element[] The structure of the lti activity.
     */
    protected function define_structure() {
        ...
        $this->add_subplugin_structure('ltiglaasterservice', $lti);
    ...
    protected function process_lti($data) {
        ...
        $newitemid = $DB->insert_record('glaaster', $data);
    ...
    protected function process_ltitype($data) {
        ...
        if (!$ltitypeid && $data->course == $courseid) {
            ...
            $ltitypeid = $DB->insert_record('glaaster_types', $data);
            $this->newltitype = true;
            $this->set_mapping('ltitype', $oldid, $ltitypeid);
        }

        // Add the typeid entry back to LTI module.
        $DB->update_record('glaaster', ['id' => $this->get_new_parentid('glaaster'), 'typeid' => $ltitypeid]);
    }
    ...
    protected function find_existing_lti_type($data) {
        ...
        if ($this->task->is_samesite()) {
            ...
            if ($DB->record_exists_select('glaaster_types', $sql, $params)) {
    ...
    protected function process_lticoursevisible($data) {
        ...
        if ($data->typeid) {
            $DB->insert_record('glaaster_coursevisible', $data);
        }
    }
    ...
    protected function process_ltitoolsetting($data) {
        ...
        $DB->insert_record('glaaster_tool_settings', $data);
    }
    ...
    protected function after_execute() {
        ...
        $this->add_related_files('mod_glaaster', 'intro', null);
    }
}
