...
namespace mod_glaaster\local\ltiglaasterservice;
...
/**
 * The mod_glaaster\local\ltiglaasterservice\service_base class.
 *
 * @package    mod_glaaster
 * @since      Moodle 2.8
 * @copyright  2014 Vital Source Technologies http://vitalsource.com
...
abstract class service_base {
    ...
    public static function get_service_path() {
        $url = new moodle_url('/mod/glaaster/services.php');
    ...
    public function get_component_id() {
        return 'ltiglaasterservice_' . $this->id;
    }
     ...
     */
    public function override_endpoint(
        string $messagetype,
    ...
    /**
     * Default implementation will check for the existence of at least one mod_glaaster entry for that tool and context.
     *
    ...
    public function is_used_in_context($typeid, $courseid) {
        ...
        $ok = $DB->record_exists('glaaster', ['course' => $courseid, 'typeid' => $typeid]);
        return $ok || $DB->record_exists('glaaster_types', ['course' => $courseid, 'id' => $typeid]);
    }
    ...
    public function is_allowed_in_context($typeid, $courseid) {
        ...
        $type = $DB->get_record('glaaster_types', ['id' => $typeid]);
