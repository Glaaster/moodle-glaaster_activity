...
namespace mod_glaaster\reportbuilder\local\entities;
...
use function mod_glaaster\reportbuilder\local\entities\get_string;
...
class tool_types extends base {
    ...
    protected function get_all_columns(): array {
        $tablealias = $this->get_table_alias('glaaster_types');
        ...
        $columns[] = (new column(
            ...
            ->add_callback(static function (string $name, stdClass $data) {
                ...
                $iconurl = $data->icon ?: $OUTPUT->image_url('monologo', 'glaaster')->out();
                $iconclass = $data->icon ? ' nofilter' : '';
                $iconcontainerclass = 'activityiconcontainer smaller';
                $name = $data->name;
                $img = html_writer::img(
                    $iconurl,
                    get_string('courseexternaltooliconalt', 'mod_glaaster', $name),
                    ['class' => 'activityicon' . $iconclass]
                );
                $name = html_writer::span($name, 'align-self-center');
                return html_writer::div(tool_types . php\html_writer::div($img, 'me-2 ' . $iconcontainerclass) . $name, 'd-flex');
            });
    ...
    protected function get_default_entity_title(): lang_string {
        return new lang_string('entitycourseexternaltools', 'mod_glaaster');
    }
}
