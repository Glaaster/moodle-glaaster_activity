...
namespace mod_glaaster\reportbuilder\local\systemreports;
...
use html_writer;
use lang_string;
use mod_glaaster\reportbuilder\local\entities\tool_types;
...
class course_external_tools_list extends system_report {
    ...
    protected function initialise(): void {
        ...
        $entitymainalias = $entitymain->get_table_alias('glaaster_types');

        $this->set_main_table('glaaster_types', $entitymainalias);
        ...
        $joinsql = "LEFT JOIN {glaaster_types_categories} {$cattablealias}
        ...
        [$insql, $params] =
            $DB->get_in_or_equal([get_site()->id, $this->course->id], SQL_PARAMS_NAMED, "{$paramprefix}_");
        ...
        $this->set_downloadable(false, get_string('pluginname', 'mod_glaaster'));
     ...
     *
     * They are all provided by the entities we previously added in the {@see initialise} method, referencing each by
    ...
    protected function add_columns(tool_types $tooltypesentity): void {
        $entitymainalias = $tooltypesentity->get_table_alias('glaaster_types');
        ...
        $this->add_column(new column(
            'usage',
            new lang_string('usage', 'mod_glaaster'),
        ...
        $this->add_column((new column(
            ...
            new lang_string('showinactivitychooser', 'mod_glaaster'),
            $tooltypesentity->get_entity_name()
        ))
            // Site tools can be overridden on course level.
            ->add_join("LEFT JOIN {glaaster_coursevisible} lc ON lc.typeid = {$entitymainalias}.id AND lc.courseid = " .
                $this->course->id)
            ->set_type(column::TYPE_INTEGER)
            ...
            ->set_callback(function (int $id, stdClass $row): string {
                ...
                $label = $coursevisible ? get_string('dontshowinactivitychooser', 'mod_glaaster')
                    : get_string('showinactivitychooser', 'mod_glaaster');

                $disabled = !has_capability('mod/glaaster:addcoursetool', context_course::instance($courseid));
        ...
        $this->add_column(new column(
            ...
            ->add_callback(function ($field, $row) {
                ...
                if (get_site()->id == $row->course) {
                    return html_writer::div(
                        html_writer::div(
                            $OUTPUT->pix_icon(
                                't/locked',
                                get_string('courseexternaltoolsnoeditpermissions', 'mod_glaaster')
                            ),
                ...
                if (!has_capability('mod/glaaster:addcoursetool', context_course::instance($row->course))) {
                    return html_writer::div(
                        html_writer::div(
                            $OUTPUT->pix_icon(
                                't/locked',
                ...
                $menu->add(new action_menu_link(
                    new moodle_url(
                        '/mod/glaaster/coursetooledit.php',
                        ['course' => $row->course, 'typeid' => $row->id]
                    ),
                    null,
    ...
    protected function can_view(): bool {
        return has_capability('mod/glaaster:addpreconfiguredinstance', $this->get_context());
    }
}
