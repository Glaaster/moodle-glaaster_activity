...
// Page setup.
$url = new moodle_url('/mod/glaaster/coursetooledit.php', ['courseid' => $courseid]);
$pageheading = !empty($typeid) ? get_string('courseexternaltooledit', 'mod_glaaster', $type->lti_typename) :
    get_string('courseexternaltooladd', 'mod_glaaster');

$PAGE->set_url($url);
$PAGE->set_pagelayout('incourse');
$PAGE->set_title($pageheading);
$PAGE->set_secondary_active_tab('coursetools_glaaster');
$PAGE->add_body_class('limitedwidth');

$form =
    new mod_glaaster_edit_types_form($url, (object) ['id' => $typeid, 'clientid' => $type->lti_clientid, 'iscoursetool' => true]);
if ($form->is_cancelled()) {
    redirect(new moodle_url('/mod/glaaster/coursetools.php', ['id' => $courseid]));
