...
use mod_glaaster\output\external_registration_return_page;
...
$pageurl = new moodle_url('/mod/glaaster/externalregistrationreturn.php');
...
if ($status !== 'success' && empty($err)) {
    ...
    if (!empty($msg)) {
        ...
        $err = get_string('failedtocreatetooltype', 'mod_glaaster');
    }
}
$params = ['message' => s($msg), 'error' => s($err), 'id' => $id, 'status' => s($status)];

$page = new external_registration_return_page();
echo $output->render($page);

$PAGE->requires->js_call_amd('mod_glaaster/external_registration_return', 'init', $params);
echo $output->footer();
