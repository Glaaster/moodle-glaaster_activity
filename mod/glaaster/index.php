...
use mod_glaaster\event\course_module_instance_list_viewed;
...
$PAGE->set_url('/mod/glaaster/index.php', ['id' => $course->id]);
...
if (!$basicltis = get_all_instances_in_course("lti", $course)) {
    notice(get_string('noltis', 'glaaster'), "../../course/view.php?id=$course->id");
    die;
}
...
foreach ($basicltis as $basiclti) {
    ...
    if ($usesections) {
        $table->data[] = [get_section_name($course, $basiclti->section), $link];
    } else {
        $table->data[] = [$link];
    }
}
