...
$foldercmid = 0;
$foldercourseid = 0;
if (!empty($filename) && !empty($filepath) && $coursemoduleid > 0) {
    $foldercmid = $coursemoduleid;
    // Get the course ID from the folder course module.
    $cm = get_coursemodule_from_id('folder', $foldercmid, 0, false, MUST_EXIST);
    $foldercourseid = $cm->course;

    // Clear resource parameters since this is a folder request.
    $resourceid = 0;
    $courseid = 0;
}
