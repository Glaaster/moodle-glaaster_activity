 ...
 * @param int $foruserid Id of the user targeted by this launch
 * @param int $resourceid Resource ID to forward as a custom claim
 * @param int $resourcecourseid Course ID of the resource
 * @param int $foldercmid Course module ID of the folder
...
function lti_glaaster_get_launch_data(
    ...
    $url = new moodle_url('/mod/glaaster/return.php', $returnurlparams);
    ...
    $plugins = core_component::get_plugin_list('ltiglaastersource');
    foreach (array_keys($plugins) as $plugin) {
        $pluginparams = component_callback(
            'ltiglaastersource_' . $plugin,
            'before_launch',
...
function lti_glaaster_build_registration_request($toolproxy) {
    ...
    // Add the return URL.
    $returnurlparams = ['id' => $toolproxy->id, 'sesskey' => sesskey()];
    $url = new moodle_url('/mod/glaaster/externalregistrationreturn.php', $returnurlparams);
    $returnurl = $url->out(false);
...
function lti_glaaster_build_request(
    ...
    // Determine download type and set parameters accordingly.
    if ($foldercmid && !empty($filename)) {
        // Folder download: set folder parameters, force resource parameters to null.
        $requestparams['folder_cmid'] = $foldercmid;
        $requestparams['folder_course_id'] = $foldercourseid;
        $requestparams['filename'] = $filename;  // Keep base64 encoded.
        $requestparams['filepath'] = !empty($filepath) ? $filepath : base64_encode('/');  // Keep base64 encoded.

        // Force resource parameters to null for clarity.
        $requestparams['resource_id'] = null;
        $requestparams['resource_course_id'] = null;
    } else if ($resourceid) {
        // Resource download: set resource parameters, force folder parameters to null.
        $requestparams['resource_id'] = $resourceid;
        $requestparams['resource_course_id'] = $resourcecourseid;

        // Force folder parameters to null for clarity.
        $requestparams['folder_cmid'] = null;
        $requestparams['folder_course_id'] = null;
        $requestparams['filename'] = null;
        $requestparams['filepath'] = null;
    }
    ...
    if (!empty($instance->cmid)) {
        $intro = format_module_intro('glaaster', $instance, $instance->cmid);
...
function lti_glaaster_build_standard_message(
    ...
    if (!empty($CFG->mod_glaaster_institution_name)) {
        $requestparams['tool_consumer_instance_name'] = trim(html_to_text($CFG->mod_glaaster_institution_name, 0));
    } else {
 ...
 * @param string $text The text to display to represent the content item. This value may be a long description of the
 *     content item.
 * @param array $mediatypes Array of MIME types types supported by the TC. If empty, the TC will support ltilink by
 *     default.
 ...
 * @param bool $autocreate Indicates whether any content items returned by the TP would be automatically persisted
 *     without
 ...
 *                       A signed message should always be required when the content item is being created
 *     automatically in the TC without further interaction from the user. False by default.
 * @param bool $canconfirm Flag for can_confirm parameter. False by default.
 * @param bool $copyadvice Indicates whether the TC is able and willing to make a local copy of a content item. False
 *     by default.
 * @param string $nonce Nonce value for LTI 1.3 launches.
 * @param int $resourceid Resource ID to forward as a custom claim.
 * @param int $resourcecourseid Course ID of the resource.
 * @return stdClass The object containing the signed request parameters and the URL to the TP's Content-Item selection
 *     interface.
...
function lti_glaaster_verify_with_keyset($jwtparam, $keyseturl, $clientid) {
    ...
    try {
        if (empty($keyset)) {
            throw new moodle_exception('errornocachedkeysetfound', 'mod_glaaster');
        }
 ...
 * @return string
 */
function glaaster_params_to_string(object $params) {
    $customparameters = [];
 ...
 */
function glaaster_content_item_to_form(object $tool, object $typeconfig, object $item): stdClass {
    global $OUTPUT;
    ...
    $config->selectcontentindicator =
        $OUTPUT->pix_icon('i/valid', get_string('yes')) . get_string('contentselected', 'mod_glaaster');
    ...
    if (!empty($items)) {
        ...
        if (count($items) == 1) {
            ...
            foreach ($items as $item) {
                $multiple[] = glaaster_content_item_to_form($tool, $typeconfig, $item);
            }
            $config = new stdClass();
...
/**
 * This function builds the tab for a category of tools
 *
 * @param array $tools Array of tool type objects
...
function lti_glaaster_get_tool_table(array $tools, string $id) {
    ...
    $typename = get_string('typename', 'glaaster');
    $baseurl = get_string('baseurl', 'glaaster');
    ...
    if (!empty($tools)) {
        ...
        foreach ($tools as $type) {
            ...
            $accept = get_string('accept', 'glaaster');
            $update = get_string('update', 'glaaster');
            $delete = get_string('delete', 'glaaster');

            if (empty($type->toolproxyid)) {
                $baseurl = new moodle_url('/mod/glaaster/typessettings.php', [
                    'action' => 'accept',
                    'id' => $type->id,
                    'sesskey' => sesskey(),
                    'tab' => $id,
                ...
                $baseurl = new moodle_url('/mod/glaaster/toolssettings.php', [
                    'action' => 'accept',
                    'id' => $type->id,
                    'sesskey' => sesskey(),
                    'tab' => $id,
            ...
            $accepthtml = $OUTPUT->action_icon(
                $baseurl,
                new pix_icon('t/check', $accept, '', ['class' => 'iconsmall']),
                null,
                ['title' => $accept, 'class' => 'editing_accept']
            );
            ...
            $updatehtml = $OUTPUT->action_icon(
                $updateurl,
                new pix_icon('t/edit', $update, '', ['class' => 'iconsmall']),
                null,
            ...
            if (($type->state != LTI_GLAASTER_TOOL_STATE_REJECTED) || empty($type->toolproxyid)) {
                ...
                $deletehtml = $OUTPUT->action_icon(
                    $deleteurl,
                    new pix_icon('t/delete', $delete, '', ['class' => 'iconsmall']),
                    null,
                    ['title' => $delete, 'class' => 'editing_delete']
                );
            } else {
                $deletehtml = '';
            }
        ...
        $html .= get_string('no_' . $id, 'glaaster');
    }
...
EOD;
        foreach ($toolproxies as $toolproxy) {
            ...
            $accept = get_string('register', 'glaaster');
            $update = get_string('update', 'glaaster');
            $delete = get_string('delete', 'glaaster');

            $baseurl = new moodle_url('/mod/glaaster/registersettings.php', [
                'action' => 'accept',
                'id' => $toolproxy->id,
                'sesskey' => sesskey(),
                'tab' => $id,
            ...
            $registerurl = new moodle_url('/mod/glaaster/register.php', [
                'id' => $toolproxy->id,
                'sesskey' => sesskey(),
                'tab' => 'tool_proxy',
            ]);
...
function lti_glaaster_calculate_custom_parameter($value) {
    ...
    switch ($value) {
        ...
        case 'Context.id.history':
            return implode(",", glaaster_get_course_history($COURSE));
 ...
 */
function glaaster_get_course_history($course) {
    global $DB;
    $history = [];
    $parentid = $course->originalcourseid;
    while (!empty($parentid) && !in_array($parentid, $history)) {
        $history[] = $parentid;
        $parentid = $DB->get_field('course', 'originalcourseid', ['id' => $parentid]);
    }
    return $history;
}
...
function lti_glaaster_get_ims_role($user, $cmid, $courseid, $islti2) {
    ...
    if (empty($cmid)) {
        ...
        if (has_capability('mod/glaaster:manage', $context)) {
            array_push($roles, 'Instructor');
        } else {
            array_push($roles, 'Learner');
        }
    ...
    if (!is_role_switched($courseid) && (is_siteadmin($user)) || has_capability('mod/glaaster:admin', $context)) {
...
function lti_glaaster_get_type_config($typeid) {
    ...
    $configs = $DB->get_records_sql(
        $query,
        ['typeid1' => $typeid, 'typeid2' => $typeid, 'typeid3' => $typeid, 'typeid4' => $typeid]
    );
...
/**
 * Returns the LTI tools matching the given URL.
 *
 * @param string $url The URL to extract the domain from
 * @param int|null $state Filter by tool state.
...
/**
 * Returns tool type records matching the given domain.
 *
...
function lti_glaaster_filter_get_types($course) {
    ...
    $query = "SELECT t.id, t.name, t.baseurl, t.state, t.toolproxyid, t.timecreated, tp.name tpname
                FROM {glaaster_types} t LEFT OUTER JOIN {glaaster_tool_proxies} tp ON t.toolproxyid = tp.id
                {$where}";
...
function lti_glaaster_get_lti_types_by_course($courseid, $coursevisible = null) {
    ...
    global $USER;
    return types_helper::get_lti_types_by_course($courseid, $USER->id, $coursevisible ?? []);
}
...
function lti_glaaster_get_types_for_add_instance() {
    ...
    $types = [(object) ['name' => get_string('automatic', 'glaaster'), 'course' => 0, 'toolproxyid' => null]];

    $preconfiguredtypes = types_helper::get_lti_types_by_course($COURSE->id, $USER->id);
 ...
 */
function glaaster_get_configured_types($courseid, $sectionreturn = 0) {
    global $OUTPUT, $USER;
    $types = [];
    ...
    foreach ($preconfiguredtypes as $ltitype) {
        ...
        $iconurl = glaaster_get_tool_type_icon_url($ltitype);
        $iconclass = '';
        if ($iconurl !== $OUTPUT->image_url('monologo', 'glaaster')->out()) {
 ...
 *
 * This function uses a regular expression to extract the domain from the provided URL.
 *
 * @param string $url The URL to extract the domain from
...
/**
 * Returns the tool type with the best match for the given URL.
 *
 * @param string $url The URL to match against
 * @param int|null $courseid The course id to filter tools by, or null for site tools
 * @param int $state The state of the tool to match against
 *
...
/**
 * Returns a thumbprint of the URL, which is the lower-case host and path, with query parameters.
 *
...
/**
 * Returns the best matching tool for the given URL.
 *
 * This function compares the thumbprint of the URL with the thumbprints of the tools.
 * It returns the tool with the highest match score, or null if no suitable tool is found.
 *
 * @param string $url The URL to match against
 * @param array $tools An array of tool objects to match against
 * @param int|null $courseid The course id to filter tools by, or null for site tools
 *
...
/**
 * Returns the shared secrets for a given key.
 *
 * This function looks up the shared secret for the specified key in both the types_config table (for configured tools)
...
function lti_glaaster_get_shared_secrets_by_key($key) {
    ...
    $query = "SELECT " . $DB->sql_compare_text('t2.value', 256) . " AS value
                ...
                JOIN {glaaster_types} type ON t2.typeid = type.id
              ...
              SELECT tp.secret AS value
                FROM {glaaster_tool_proxies} tp
                JOIN {glaaster_types} t ON tp.id = t.toolproxyid
              WHERE tp.guid = :key2
...
function lti_glaaster_delete_type($id) {
    ...
    $DB->delete_records('glaaster_types_config', ['typeid' => $id]);
    $DB->delete_records('glaaster_types_categories', ['typeid' => $id]);
}

/**
 * Set the state of a Basic LTI type
 *
 * @param int $id Configuration id
...
function lti_glaaster_get_type_type_config($id) {
    ...
    foreach ($config as $name => $value) {
        if (strpos($name, 'ltiglaasterservice_') === 0) {
...
/**
 * Prepares the type object for saving to the database.
 *
 * @param stdClass $type The type object to prepare
 ...
 *
 * @param stdClass $type The type object to update
 * @param stdClass $config The configuration object containing the settings
 * @return void
 */
function lti_glaaster_update_type(stdClass $type, stdClass $config) {
    ...
    if ($DB->update_record('glaaster_types', $type)) {
        ...
        if (isset($type->toolproxyid) && $type->ltiversion === LTI_GLAASTER_VERSION_1P3) {
            ...
            $DB->delete_records('glaaster_tool_proxies', ['id' => $toolproxyid]);
            $type->toolproxyid = null;
            $DB->update_record('glaaster_types', $type);
        }
        ...
        if ($clearcache) {
            $sql = "SELECT cm.id, cm.course
                      ...
                      JOIN {glaaster} l ON l.course = cm.course
...
function lti_glaaster_type_add_categories(int $typeid, string $lticoursecategories = ''): void {
    ...
    foreach ($coursecategories as $coursecategory) {
        $DB->insert_record('glaaster_types_categories', ['typeid' => $typeid, 'categoryid' => $coursecategory]);
    }
}
...
function lti_glaaster_add_type($type, $config) {
    ...
    $id = $DB->insert_record('glaaster_types', $type);
    ...
    if ($id) {
        foreach ($config as $key => $value) {
            if (!is_null($value)) {
                ...
                } else if (substr($key, 0, 11) !== 'ltiglaasterservice_') {
                    continue;
                } else {
                    $fieldname = $key;
                }
...
function lti_glaaster_get_tool_proxies_from_registration_url($regurl) {
    ...
    return $DB->get_records_sql(
        'SELECT * FROM {glaaster_tool_proxies}
...
function lti_glaaster_get_tool_proxies($orphanedonly) {
    ...
    if ($orphanedonly) {
        $usedproxyids =
            array_values($DB->get_fieldset_select('glaaster_types', 'toolproxyid', 'toolproxyid IS NOT NULL'));
        $proxies = $DB->get_records('glaaster_tool_proxies', null, 'state DESC, timemodified DESC');
        ...
        return $DB->get_records('glaaster_tool_proxies', null, 'state DESC, timemodified DESC');
    }
}
...
function lti_glaaster_add_tool_proxy($config) {
    ...
    if (isset($config->toolproxyid) && !empty($config->toolproxyid)) {
        ...
        $toolproxy->guid = random_string();
        $toolproxy->secret = random_string();
        $id = $DB->insert_record('glaaster_tool_proxies', $toolproxy);
    }
...
function lti_glaaster_update_tool_proxy($toolproxy) {
    ...
    $toolproxy->timemodified = time();
    $id = $DB->update_record('glaaster_tool_proxies', $toolproxy);
...
function lti_glaaster_get_lti_types_and_proxies(
    ...
    if ($orphanedonly) {
        $orphanedproxiessql = helper::glaaster_get_tool_proxy_sql($orphanedonly, false);
        $countsql = helper::glaaster_get_tool_proxy_sql($orphanedonly, true);
        $proxies = $DB->get_records_sql($orphanedproxiessql, null, $offset, $limit);
        $totalproxiescount = $DB->count_records_sql($countsql);
    } else {
        $proxies = $DB->get_records(
            'glaaster_tool_proxies',
            null,
            'name ASC, state DESC, timemodified DESC',
            '*',
            $offset,
            $limit
        );
        $totalproxiescount = $DB->count_records('glaaster_tool_proxies');
    }
    ...
    return [$proxies, array_map('glaaster_serialise_tool_type', $types)];
}
...
function lti_glaaster_get_lti_types_and_proxies_count(bool $orphanedonly = false, int $toolproxyid = 0): int {
    ...
    $proxiessql = helper::glaaster_get_tool_proxy_sql($orphanedonly, true);
...
function lti_glaaster_update_config($config) {
    ...
    if ($old) {
        $config->id = $old->id;
        $return = $DB->update_record('glaaster_types_config', $config);
    } else {
        $return = $DB->insert_record('glaaster_types_config', $config);
    }
    return $return;
...
function lti_glaaster_get_tool_settings($toolproxyid, $courseid = null, $instanceid = null) {
    ...
    $settings = [];
    if ($toolproxyid > 0) {
        $settingsstr = $DB->get_field('glaaster_tool_settings', 'settings', [
            'toolproxyid' => $toolproxyid,
            'course' => $courseid,
            'coursemoduleid' => $instanceid,
        ]);
    } else {
        $settingsstr = $DB->get_field('glaaster_tool_settings', 'settings', [
            'typeid' => -$toolproxyid,
...
function lti_glaaster_set_tool_settings($settings, $toolproxyid, $courseid = null, $instanceid = null) {
    ...
    if ($toolproxyid >= 0) {
        $record = $DB->get_record('glaaster_tool_settings', [
            'toolproxyid' => $toolproxyid,
            'course' => $courseid,
            'coursemoduleid' => $instanceid,
        ]);
    ...
    if ($record !== false) {
        $DB->update_record(
            ...
            (object) ['id' => $record->id, 'settings' => $json, 'timemodified' => time()]
        );
    } else {
        $record = new stdClass();
        if ($toolproxyid > 0) {
        ...
        $DB->insert_record('glaaster_tool_settings', $record);
    }
}
...
/**
 * Converts the message paramters to their equivalent JWT claim and signs the payload to launch the external tool using
 * JWT
 *
...
function lti_glaaster_sign_jwt($parms, $endpoint, $oauthconsumerkey, $typeid = 0, $nonce = '') {
    ...
    if (isset($parms['folder_course_id'])) {
        $payload['folder_course_id'] = $parms['folder_course_id'];
    }
    if (isset($parms['filename'])) {
        $payload['filename'] = $parms['filename'];
    }
...
function lti_glaaster_build_login_request(
    ...
    if (!empty($instance)) {
        ...
        $SESSION->$launchid =
            "{$courseid},{$config->typeid},,{$messagetype},{$foruserid}," . base64_encode($title) . ',' .
            base64_encode($text);
    }
    $endpoint = trim($endpoint);
    $services = lti_glaaster_get_services();
    ...
    $ltihint['resourcecourseid'] = $resourcecourseid;
    $ltihint['folderCmid'] = $foldercmid;
    $ltihint['folderCourseId'] = $foldercourseid;
    if (!empty($filename)) {
        $ltihint['filename'] = $filename;
    }
    if (!empty($filepath)) {
...
/**
 * Checks if the request is using SSL.
 *
...
/**
 * Ensures that the URL is using HTTPS.
 *
...
function lti_glaaster_should_log_request($rawbody) {
    ...
    $logusers = explode(',', $CFG->mod_glaaster_log_users);
    if (empty($logusers)) {
        return false;
    }
    ...
    try {
        ...
        $ns = array_shift($ns);
        $xml->registerXPathNamespace('glaaster', $ns);
        $requestuserid = '';
...
function lti_glaaster_get_services() {
    ...
    foreach ($definedservices as $name => $location) {
        $classname = "\\ltiglaasterservice_{$name}\\local\\service\\{$name}";
 ...
 *
 * @param service_base[] $services Array of services
 ...
 */
function glaaster_get_tool_type_icon_url(stdClass $type) {
    global $OUTPUT;
    ...
    if (empty($iconurl)) {
        $iconurl = $OUTPUT->image_url('monologo', 'glaaster')->out();
    }
 ...
 */
function glaaster_get_tool_type_edit_url(stdClass $type) {
    $url = new moodle_url(
        '/mod/glaaster/typessettings.php',
        ['action' => 'update', 'id' => $type->id, 'sesskey' => sesskey(), 'returnto' => 'toolconfigure']
    );
    return $url->out();
}
 ...
 * @return string The url to edit the tool type
 */
function glaaster_get_tool_proxy_edit_url(stdClass $proxy) {
    $url = new moodle_url(
        '/mod/glaaster/registersettings.php',
        ['action' => 'update', 'id' => $proxy->id, 'sesskey' => sesskey(), 'returnto' => 'toolconfigure']
    );
    return $url->out();
}
 ...
 */
function glaaster_get_tool_type_course_url(stdClass $type) {
    if ($type->course != 1) {
        $url = new moodle_url('/course/view.php', ['id' => $type->course]);
        return $url->out();
    }
    return null;
}
 ...
 */
function glaaster_get_tool_type_urls(stdClass $type) {
    $courseurl = glaaster_get_tool_type_course_url($type);

    $urls = [
        'icon' => glaaster_get_tool_type_icon_url($type),
        'edit' => glaaster_get_tool_type_edit_url($type),
    ];
    ...
    $url = new moodle_url('/mod/glaaster/certs.php');
    $urls['publickeyset'] = $url->out();
    $url = new moodle_url('/mod/glaaster/token.php');
    $urls['accesstoken'] = $url->out();
    $url = new moodle_url('/mod/glaaster/auth.php');
 ...
 */
function glaaster_get_tool_proxy_urls(stdClass $proxy) {
    global $OUTPUT;

    $urls = [
        'icon' => $OUTPUT->image_url('monologo', 'glaaster')->out(),
        'edit' => glaaster_get_tool_proxy_edit_url($proxy),
    ];
 ...
 */
function glaaster_get_tool_type_state_info(stdClass $type) {
    $isconfigured = false;
    $ispending = false;
    $isrejected = false;
    $isunknown = false;
    switch ($type->state) {
        ...
        case LTI_GLAASTER_TOOL_STATE_REJECTED:
            ...
            break;
        default:
            $state = get_string('unknownstate', 'mod_glaaster');
            $isunknown = true;
            break;
    }
...
function glaaster_glaaster_get_tool_type_config($type) {
    ...
    $publickeyseturl = new moodle_url('/mod/glaaster/certs.php');
    $publickeyseturl = $publickeyseturl->out();

    $accesstokenurl = new moodle_url('/mod/glaaster/token.php');
    $accesstokenurl = $accesstokenurl->out();

    $authrequesturl = new moodle_url('/mod/glaaster/auth.php');
...
function glaaster_glaaster_get_tool_type_capability_groups($type) {
    ...
    foreach ($capabilities as $capability) {
        ...
        if (!$hascourse && preg_match('/^CourseSection/', $capability)) {
            $hascourse = true;
            $groups[] = get_string('courseinformation', 'mod_glaaster');
        } else if (!$hasactivities && preg_match('/^ResourceLink/', $capability)) {
            $hasactivities = true;
            $groups[] = get_string('courseactivitiesorresources', 'mod_glaaster');
        } else if (!$hasuseraccount && preg_match('/^User/', $capability) || preg_match('/^Membership/', $capability)) {
            $hasuseraccount = true;
            $groups[] = get_string('useraccountinformation', 'mod_glaaster');
        } else if (!$hasuserpersonal && preg_match('/^Person/', $capability)) {
            $hasuserpersonal = true;
            $groups[] = get_string('userpersonalinformation', 'mod_glaaster');
        }
    }
...
function glaaster_serialise_tool_type(stdClass $type) {
    ...
    if (!empty($type->description)) {
        ...
        $description = get_string('editdescription', 'mod_glaaster');
    }
    $tooldomain = !empty($type->tooldomain) ? $type->tooldomain : (get_config('mod_glaaster', 'tooldomain') ?: 'lti.glaaster.com');
    return [
        'id' => $type->id,
        'name' => $name,
        'description' => $description,
        'urls' => glaaster_get_tool_type_urls($type),
        'state' => glaaster_get_tool_type_state_info($type),
        'platformid' => $CFG->wwwroot,
        'clientid' => $type->clientid,
        'deploymentid' => $type->id,
        'statusurl' => 'https://' . $tooldomain . '/status',
...
function lti_glaaster_load_cartridge($url, $map, $propertiesmap = []) {
    ...
    foreach ($map as $tag => $key) {
        $value = glaaster_get_tag($tag, $cartridge);
        if ($value) {
            $toolinfo[$key] = $value;
        }
    }
    if (!empty($propertiesmap)) {
        foreach ($propertiesmap as $property => $key) {
            $value = glaaster_get_tag("property", $cartridge, $property);
            if ($value) {
                $toolinfo[$key] = $value;
            }
        }
 ...
 * @since Moodle 3.1
 */
function glaaster_get_tag($tagname, $xpath, $attribute = null) {
    if ($attribute) {
...
function lti_glaaster_new_access_token($typeid, $scopes) {
    ...
    } while ($DB->record_exists('glaaster_access_tokens', ['token' => $generatedtoken]));
...
/**
 * Retrieve a valid Glaaster instance ID based on the tool domain.
 *
 * Validates that the instance exists, is not deleted, and belongs to an active course.
 * Supports multiple instances - returns first valid one found.
 *
 * Validation checks:
 * - Tool type exists
 * - Instance exists
 ...
 *
 * @return int|false Returns the first valid instance ID, or false if none found.
 * @since Moodle 4.1
 */
function glaaster_retrieve_instance_from_tooldomain() {
    global $DB;
    ...
    // Retrieve all instances (supports multiple instances, returns first valid).
    $instances = $DB->get_records('glaaster', ['typeid' => $type->id]);
    if (empty($instances)) {
        return false;
    }

    // Validate each instance and return first valid one.
    foreach ($instances as $instance) {
        ...
        // Check course module exists and is not being deleted.
        // deletioninprogress = 0 is critical - catches deletions immediately.
        $cmexists = $DB->record_exists_sql(
            "SELECT cm.id
             FROM {course_modules} cm
             WHERE cm.course = ?
               AND cm.module = ?
...
/**
 * Check if Moodle web services are enabled and functional.
 *
 * Verifies that web services are enabled globally in Moodle configuration.
 * This is required for AJAX calls to external functions to work properly.
 *
 * @return bool True if web services are enabled, false otherwise
 */
function glaaster_check_webservices_enabled() {
    // Check if web services are enabled globally.
    $webservicesenabled = get_config('core', 'enablewebservices');

    // Return true if enabled (value is 1), false otherwise.
    return !empty($webservicesenabled);
}
...
/**
 * Build a sourcedid object for grade service authentication.
 *
