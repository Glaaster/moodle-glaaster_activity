...
/**
 * The form for adding/editing an instance of the LTI module.
 *
 * This form is used to create a new instance of the LTI module, or to edit an existing instance.
 * It supports both preconfigured tools and manually configured tools (deprecated in 4.3).
 *
 * @package mod_glaaster
 */
class mod_glaaster_mod_form extends moodleform_mod {
     ...
     * Constructor.
     *
     * Throws an exception if trying to init the form for a new manual instance of a tool, which is not supported in
     * 4.3 onward.
     *
     ...
     * @throws moodle_exception if trying to init the form for the creation of a manual instance, which is no longer
     *     supported.
     */
    public function __construct($current, $section, $cm, $course) {
        ...
        if ($PAGE->has_set_url() && str_contains($PAGE->url, '/course/modedit.php')) {
            if (empty($this->typeid) && empty($current->id)) {
                throw new moodle_exception('lti:addmanualinstanceprohibitederror', 'mod_glaaster');
            }
        }
    ...
    /**
     * Defines the form elements for the LTI module instance.
     *
     * This method is called to define the form elements for the LTI module instance.
     * It sets up the fields, validation rules, and other properties of the form.
     *
     * @return void
     */
    public function definition(): void {
        ...
        if ($this->type) {
            component_callback("ltiglaastersource_$this->type", 'add_instance_hook');
        }
        ...
        $mform->addElement(
            'html',
            "<div data-attribute='dynamic-import' hidden aria-hidden='true' role='alert'></div>"
        );
        ...
        if ($supportscontentitemselection) {
            $contentitemurl = new moodle_url('/mod/glaaster/contentitem.php');
            ...
            $selectcontentindicatorinner = $iscontentitem ?
                $OUTPUT->pix_icon(
                    'i/valid',
                    get_string('contentselected', 'mod_glaaster'),
                    'moodle',
                    ['class' => 'me-1']
                )
                . get_string('contentselected', 'mod_glaaster') : '';
            $selectcontentindicator = html_writer::div(
                $selectcontentindicatorinner,
                '',
            ...
            $selectcontentgrp = [
                $mform->createElement(
                    'button',
                    'selectcontent',
                    get_string('selectcontent', 'mod_glaaster'),
                    $contentbuttonattributes,
        ...
        $mform->addElement('text', 'name', get_string('basicltiname', 'glaaster'), ['size' => '64']);
        ...
        if (
            ...
            $mform->addHelpButton('showtitlelaunch', 'display_name', 'glaaster');
        } else {
        ...
        $this->standard_intro_elements(get_string('basicltiintro', 'glaaster'));
        ...
        if (
            ...
            $mform->addHelpButton('showdescriptionlaunch', 'display_description', 'glaaster');
        } else {
            // Include in the form anyway, so we retain the setting value in case the tool launch container is changed back.
        ...
        $mform->addHelpButton('instructorcustomparameters', 'custom', 'glaaster');
        ...
        if ($supportscontentitemselection) {
            $PAGE->requires->js_call_amd('mod_glaaster/mod_form', 'init', [$COURSE->id]);
        }
    }

    /**
     * Defines the form for legacy instances. Here tool config is frozen because the manual configuration method is
     * deprecated.
     *
    ...
    protected function legacy_instance_form_definition(array $instancetypes): void {
        ...
        // Since 'mod/glaaster:addmanualinstance' capability is deprecated, determining which users may have had access
        // to the certain form fields (the manual config fields) isn't straightforward.
        // Users without 'mod/glaaster:addmanualinstance' would have only
        // been permitted to edit the basic instance fields (name, etc.), so care must be taken not to display
        // the config fields to these users. Users who can add/edit course tools (mod/glaaster:addcoursetool)
        // are able to view tool information anyway, via the tool definitions, so this capability is used as a
        // replacement to control access to these tool config fields.
        $canviewmanualconfig = has_capability('mod/glaaster:addcoursetool', $this->context);
        $showtypes = has_capability('mod/glaaster:addpreconfiguredinstance', $this->context);
        ...
        $mform->addElement(
            'html',
            $OUTPUT->notification(
                get_string('editmanualinstancedeprecationwarning', 'mod_glaaster', get_docs_url('External_tool')),
        ...
        if ($showtypes) {
            if ($manualinstance) {
                ...
                $tooltypes = $mform->addElement('select', 'typeid', get_string('external_tool_type', 'glaaster'));
                $mform->addHelpButton('typeid', 'external_tool_type', 'glaaster');
                ...
                $mform->addElement(
                    'static',
                    'typeiddisplayonly',
                    get_string('external_tool_type', 'glaaster'),
        ...
        $mform->addElement(
            'button',
            'selectcontent',
            get_string('selectcontent', 'glaaster'),
            ['disabled' => 'disabled']
        );
        $mform->disabledIf('selectcontent', 'typeid', 'eq', $typeid);

        if ($canviewmanualconfig) {
            $mform->addElement('text', 'toolurl', get_string('launch_url', 'glaaster'), ['size' => '64']);
            $mform->setType('toolurl', PARAM_URL);
            $mform->addHelpButton('toolurl', 'launch_url', 'glaaster');

            $mform->addElement('text', 'securetoolurl', get_string('secure_launch_url', 'glaaster'), ['size' => '64']);
            $mform->setType('securetoolurl', PARAM_URL);
            $mform->setAdvanced('securetoolurl');
            $mform->addHelpButton('securetoolurl', 'secure_launch_url', 'glaaster');
        } else {
            // Need to submit these still, but hidden to avoid instructor modification.
            $mform->addElement('hidden', 'toolurl', '', ['id' => 'id_toolurl']);
            $mform->setType('toolurl', PARAM_URL);
        ...
        if ($canviewmanualconfig) {
            $mform->addElement('text', 'resourcekey', get_string('resourcekey', 'glaaster'));
            $mform->setType('resourcekey', PARAM_TEXT);
            $mform->setAdvanced('resourcekey');
            $mform->addHelpButton('resourcekey', 'resourcekey', 'glaaster');
            $mform->setForceLtr('resourcekey');

            $mform->addElement('passwordunmask', 'password', get_string('password', 'glaaster'));
            $mform->setType('password', PARAM_TEXT);
            $mform->setAdvanced('password');
            $mform->addHelpButton('password', 'password', 'glaaster');
            ...
            $mform->addElement('text', 'icon', get_string('icon_url', 'glaaster'), ['size' => '64']);
            $mform->setType('icon', PARAM_URL);
            $mform->setAdvanced('icon');
            $mform->addHelpButton('icon', 'icon_url', 'glaaster');

            $mform->addElement('text', 'secureicon', get_string('secure_icon_url', 'glaaster'), ['size' => '64']);
            $mform->setType('secureicon', PARAM_URL);
            $mform->setAdvanced('secureicon');
            $mform->addHelpButton('secureicon', 'secure_icon_url', 'glaaster');
        } else {
            // Need to submit these still, but hidden to avoid instructor modification.
            $mform->addElement('hidden', 'resourcekey', '', ['id' => 'id_resourcekey']);
            $mform->setType('resourcekey', PARAM_TEXT);
        ...
        $mform->addElement('advcheckbox', 'instructorchoicesendname', get_string('share_name', 'glaaster'));
        $mform->addHelpButton('instructorchoicesendname', 'share_name', 'glaaster');

        $mform->addElement('advcheckbox', 'instructorchoicesendemailaddr', get_string('share_email', 'glaaster'));
        $mform->addHelpButton('instructorchoicesendemailaddr', 'share_email', 'glaaster');
