...
/**
 * The mod_glaaster_register_types_form class.
 *
 * @package    mod_glaaster
 * @since      Moodle 2.8
 * @copyright  2014 Vital Source Technologies http://vitalsource.com
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_glaaster_register_types_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'setup', get_string('registration_options', 'glaaster'));

        // Tool Provider name.

        $strrequired = get_string('required');
        $mform->addElement('text', 'lti_registrationname', get_string('registrationname', 'glaaster'));
        $mform->setType('lti_registrationname', PARAM_TEXT);
        $mform->addHelpButton('lti_registrationname', 'registrationname', 'glaaster');
        $mform->addRule('lti_registrationname', $strrequired, 'required', null, 'client');

        // Registration URL.

        $mform->addElement('text', 'lti_registrationurl', get_string('registrationurl', 'glaaster'), ['size' => '64']);
        $mform->setType('lti_registrationurl', PARAM_URL);
        $mform->addHelpButton('lti_registrationurl', 'registrationurl', 'glaaster');
        ...
        $mform->addElement(
            'select',
            'lti_capabilities',
            get_string('capabilities', 'glaaster'),
            array_combine($options, $options),
            $attributes
        );
        $mform->setType('lti_capabilities', PARAM_TEXT);
        $mform->addHelpButton('lti_capabilities', 'capabilities', 'glaaster');
        ...
        $attributes = ['multiple' => 1, 'size' => min(count($options), 10)];
        $mform->addElement('select', 'lti_services', get_string('services', 'glaaster'), $options, $attributes);
        $mform->setType('lti_services', PARAM_TEXT);
        $mform->addHelpButton('lti_services', 'services', 'glaaster');
