...
if (empty($top)) {
    ...
    $redirect = new moodle_url('/mod/glaaster/registrationreturn.php', $params);
    $redirect = $redirect->out(false);

    $clickhere = get_string('click_to_continue', 'glaaster', (object) ['link' => $redirect]);
    $html = <<<EOD
<html>
<head>
...
EOD;
    ...
    if (!empty($id)) {
        ...
        switch ($toolproxy->state) {
            case LTI_GLAASTER_TOOL_PROXY_STATE_ACCEPTED:
                $redirect->param('tab', 'tp_accepted');
                break;
            case LTI_GLAASTER_TOOL_PROXY_STATE_REJECTED:
                $redirect->param('tab', 'tp_rejected');
                break;
            case LTI_GLAASTER_TOOL_PROXY_STATE_PENDING:
