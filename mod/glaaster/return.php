...
if (!empty($errormsg) || !empty($msg)) {
    $url = new moodle_url('/mod/glaaster/return.php', ['course' => $courseid]);
    $PAGE->set_url($url);
...
if (!empty($errormsg)) {
    echo get_string('lti_launch_error', 'glaaster');
    p($errormsg);
    ...
    if ($unsigned == 1) {
        ...
        if (has_capability('mod/glaaster:addcoursetool', $contextcourse)) {
            $coursetooleditor = new moodle_url('mod/glaaster/coursetools.php', ['id' => $courseid]);
            $links->course_tool_editor = $coursetooleditor->out(false);

            echo get_string('lti_launch_error_unsigned_help', 'glaaster', $links);
        }

        if (!empty($lti) && has_capability('mod/glaaster:requesttooladd', $contextcourse)) {
            $adminrequesturl =
                new moodle_url('/mod/glaaster/request_tool.php', ['instanceid' => $lti->id, 'sesskey' => sesskey()]);
            $links->admin_request_url = $adminrequesturl->out(false);

            echo get_string('lti_launch_error_tool_request', 'glaaster', $links);
        }
    }
