...
namespace ltiglaasterservice_memberships\local\resources;

use context_course;
use mod_glaaster\local\ltiglaasterservice\resource_base;
use ltiglaasterservice_memberships\local\service\memberships;
...
class linkmemberships extends resource_base {
     ...
     * Class constructor.
     *
     * @param memberships $service Service instance
     */
    ...
    public function execute($response) {
        ...
        if (!($lti = $DB->get_record('glaaster', ['id' => $linkid], 'id,course,typeid,servicesalt', IGNORE_MISSING))) {
            $response->set_code(404);
            return;
        }
        ...
        $cm = get_coursemodule_from_instance('glaaster', $linkid, $lti->course, false, MUST_EXIST);
        $cm = $modinfo->get_cm($cm->id);
        $info = new info_module($cm);
        if ($info->is_available_for_all()) {
            $info = null;
        }
        $json = $this->get_service()->get_members_json(
            $this,
            $context,
            $course,
            $role,
    ...
    public function get_permissions($typeid) {
        ...
        if ($tool->memberships == '1') {
            return ['ToolProxyBinding.memberships.url:get'];
        } else {
            return [];
        }
    }
