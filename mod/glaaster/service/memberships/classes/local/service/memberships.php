 ...
 *
 * @package    ltiglaasterservice_memberships
 * @copyright  2015 Vital Source Technologies http://vitalsource.com
 * @author     Stephen Vickers
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace ltiglaasterservice_memberships\local\service;

use context_course;
use core_availability\info_module;
use course;
use ltiglaasterservice_memberships\local\resources\contextmemberships;
use ltiglaasterservice_memberships\local\resources\linkmemberships;
use mod_glaaster\local\ltiglaasterservice\resource_base;
use mod_glaaster\local\ltiglaasterservice\response;
...
class memberships extends service_base {
    ...
    const INSTRUCTOR_DELEGATE_INCLUDED = [self::DELEGATE_TO_INSTRUCTOR && self::INSTRUCTOR_INCLUDED];
     ...
     *
     * @param resource_base $resource Resource handling the request
     ...
     * @see memberships::get_members_json($resource, $context, $course, $role, $limitfrom, $limitnum, $lti, $info,
     *     $response)
     */
    public static function get_users_json(
        $resource,
        $context,
        $contextid,
        $tool,
        ...
        $memberships->check_tool($tool->id, null, [self::SCOPE_MEMBERSHIPS_READ]);

        $response = new response();

        $json = $memberships->get_members_json(
            $resource,
            $context,
            $course,
            $role,
     ...
     * @param info_module $info Conditional availability information
     *      for LTI instance (null if context-level request)
     * @param response $response Response object for the request
     *
     * @return string
     */
     ...
     * @param int $limitnum Maximum number of records to be returned
     * @param object $lti LTI instance record
     * @param info_module $info Conditional availability information for LTI instance
     * @param response $response Response object for the request
     *
     * @return string
     */
        ...
        foreach ($users as $user) {
            ...
            if (!is_null($lti)) {
                ...
                if (!empty($lti->servicesalt) && $DB->record_exists('grade_items', $conditions)) {
                    ...
                    $serviceurl = new moodle_url('/mod/glaaster/service.php');
    ...
    public function get_launch_parameters($messagetype, $courseid, $user, $typeid, $modlti = null) {
        ...
        if (isset($tool->{$this->get_component_id()})) {
            if (
                ...
                $this->is_used_in_context($typeid, $courseid)
            ) {
