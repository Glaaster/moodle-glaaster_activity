...
namespace ltiglaasterservice_toolsettings\local\resources;

use ltiglaasterservice_toolsettings\local\service\toolsettings;
 ...
 *
 * This class represents the Context Settings resource for LTI tool settings.
 *
 * @package ltiglaasterservice_toolsettings
 */
        ...
        if (!$ok) {
            ...
            if (!$ok) {
                ...
                if ($response->get_request_method() == 'GET') {
                    ...
                    $settings =
                        toolsettings::settings_to_json($systemsettings, $simpleformat, 'ToolProxy', $systemsetting);
                    $json .= $settings;
                    $isfirst = strlen($settings) <= 0;
                    $settings =
                        toolsettings::settings_to_json($contextsettings, $simpleformat, 'ToolProxyBinding', $this);
                    ...
                    if ($response->get_content_type() == $this->formats[0]) {
                        ...
                        $ok = !empty($json);
                        if ($ok) {
                            $ok = isset($json->{"@graph"}) && is_array($json->{"@graph"}) &&
                                (count($json->{"@graph"}) == 1) &&
