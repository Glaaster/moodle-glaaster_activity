...
use mod_glaaster\output\registration_upgrade_choice_page;
...
$types = lti_glaaster_get_tools_by_url($starturl, null);

if (!empty($types) && $typeid == -1) {
    // There are matching types for the registration domain, let's prompt the user to upgrade.
    $pageurl = new moodle_url('/mod/glaaster/startltiadvregistration.php');
    $PAGE->set_context($context);
    $PAGE->set_url($pageurl);
    $PAGE->set_pagelayout('maintenance');
    $output = $PAGE->get_renderer('mod_glaaster');
    $page = new registration_upgrade_choice_page($types, $starturl);
    echo $output->header();
    ...
    $confurl = new moodle_url('/mod/glaaster/openid-configuration.php');
