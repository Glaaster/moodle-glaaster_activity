@mod @mod_glaaster
Feature: Glaaster - Manage course tools
  ...
  # The first tool isn't visible in courses, the next two are, and the last tool is in a pending state and is not visible.
    Given the following "mod_glaaster > tool types" exist:
  ...
  Scenario: Viewing course tools without the capability to add/edit but having the capability to use
    Given the following "role capability" exists:
      | role                                  | editingteacher |
      | mod/glaaster:addcoursetool            | prohibit       |
      | mod/glaaster:addpreconfiguredinstance | allow          |
  ...
  Scenario: Viewing course tools with the capability to add/edit and without the capability to use
    Given the following "role capability" exists:
      | role                                  | editingteacher |
      | mod/glaaster:addcoursetool            | allow          |
      | mod/glaaster:addpreconfiguredinstance | prohibit       |
    And the following "mod_glaaster > course tools" exist:
      | name      | description         | baseurl                  | course |
      | Test tool | Example description | https://example.com/tool | C1     |
  ...
  Scenario: Edit a course tool
    Given the following "mod_glaaster > course tools" exist:
      | name      | description         | baseurl                  | course |
      | Test tool | Example description | https://example.com/tool | C1     |
    And I am on the "Course 1" course page logged in as teacher1
    And I navigate to "LTI External tools" in current page administration
  ...
  Scenario: Navigate through the listing of course tools
    Given 20 "mod_glaaster > course tools" exist with the following data:
  ...
  Scenario: Add a course tool using a cartridge URL
    ...
    And I set the field "Tool URL" to local url "/mod/glaaster/tests/fixtures/ims_cartridge_basic_lti_link.xml"
    And I press "Save changes"
    Then I should see "Test tool 1" in the "reportbuilder-table" "table"
  ...
  Scenario: Site tool appearing in activity chooser according to settings
    Given the following "mod_glaaster > tool types" exist:
      ...
      | Teaching Tool 1 | /mod/glaaster/tests/fixtures/tool_provider.php | 2             | 1     |
      | Teaching Tool 2 | /mod/glaaster/tests/fixtures/tool_provider.php | 1             | 1     |
      | Teaching Tool 3 | /mod/glaaster/tests/fixtures/tool_provider.php | 0             | 1     |
  ...
  Scenario: Course tool appearing in activity chooser according to settings
    Given the following "mod_glaaster > course tools" exist:
      ...
      | Course Tool 1 | /mod/glaaster/tests/fixtures/tool_provider.php | C1     | 2             |
      | Course Tool 2 | /mod/glaaster/tests/fixtures/tool_provider.php | C1     | 1             |
    And I log in as "teacher1"
    And I am on "Course 1" course homepage with editing mode on
    And I open the activity chooser
