@mod @mod_glaaster
Feature: Glaaster - Configure tool types
  ...
  Scenario: Add a tool type from a cartridge URL
    When I set the field "url" to local url "/mod/glaaster/tests/fixtures/ims_cartridge_basic_lti_link.xml"
  ...
  Scenario: Try to add a non-existant cartridge
    When I set the field "url" to local url "/mod/glaaster/tests/fixtures/nonexistant.xml"
    And I press "Add Legacy LTI"
    Then I should see "Enter your consumer key and shared secret"
    And I press "Save changes"
  ...
  Scenario: Attempt to add a tool type from a configuration URL, then cancel
    When I set the field "url" to local url "/mod/glaaster/tests/fixtures/tool_provider.php"
    And I press "Add Legacy LTI"
