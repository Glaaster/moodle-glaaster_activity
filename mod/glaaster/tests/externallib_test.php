 ...
 * @since      Moodle 3.0
 */
final class externallib_test extends mod_glaaster_testcase {
    /**
     ...
     */
    public function test_mod_glaaster_get_tool_proxies(): void {
        ...
        // Fetch all proxies.
        $proxies = mod_glaaster_external::get_tool_proxies(false);
        $proxies = external_api::clean_returnvalue(mod_glaaster_external::get_tool_proxies_returns(), $proxies);
     ...
     */
    public function test_mod_glaaster_get_orphaned_tool_proxies(): void {
        // Create two tool proxies. One to associate with tool, and one to leave orphaned.
        $this->setAdminUser();
        $proxy = $this->generate_tool_proxy("1");
        $orphanedproxy = $this->generate_tool_proxy("2");
        ...
        // Fetch all proxies.
        $proxies = mod_glaaster_external::get_tool_proxies(true);
        $proxies = external_api::clean_returnvalue(mod_glaaster_external::get_tool_proxies_returns(), $proxies);
    ...
    public function test_get_tool_launch_data(): void {
        ...
        $result = mod_glaaster_external::get_tool_launch_data($lti->id);
        $result = external_api::clean_returnvalue(mod_glaaster_external::get_tool_launch_data_returns(), $result);
    ...
    public function test_mod_glaaster_get_ltis_by_courses(): void {
        ...
        $lti2 = self::getDataGenerator()->create_module('glaaster', $record);
        ...
        $result = mod_glaaster_external::get_ltis_by_courses([$course2->id]);
        $result = external_api::clean_returnvalue($returndescription, $result);
        ...
        $result = mod_glaaster_external::get_ltis_by_courses([$course->id]);
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedltis, $result['ltis']);
        ...
        assign_capability('mod/glaaster:view', CAP_PROHIBIT, $studentrole->id, $contextcourse1->id);
        // Empty all the caches that may be affected by this change.
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();

        $ltis = mod_glaaster_external::get_ltis_by_courses([$course->id]);
        $ltis = external_api::clean_returnvalue(mod_glaaster_external::get_ltis_by_courses_returns(), $ltis);
    ...
    public function test_view_lti_invalid_instanceid(): void {
        ...
        mod_glaaster_external::view_lti(0);
    }
    ...
    public function test_view_lti_no_enrolment(): void {
        ...
        $this->expectException(moodle_exception::class);
        mod_glaaster_external::view_lti($lti->id);
    }

    /**
     * Test view_lti for a user without the mod/glaaster:view capability.
     */
    public function test_view_lti_no_capability(): void {
        ...
        assign_capability('mod/glaaster:view', CAP_PROHIBIT, $studentrole->id, $context->id);
        // Empty all the caches that may be affected by this change.
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();
    ...
    /**
     * Test view_lti for a user with the mod/glaaster:view capability in the course.
     */
    public function test_view_lti(): void {
        ...
        $result = mod_glaaster_external::view_lti($lti->id);
        // The value of the result isn't needed but validation is.
        external_api::clean_returnvalue(mod_glaaster_external::view_lti_returns(), $result);
        ...
        $this->assertInstanceOf('\mod_glaaster\event\course_module_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $moodlelti = new moodle_url('/mod/glaaster/view.php', ['id' => $cm->id]);
     ...
     */
    public function test_mod_glaaster_create_tool_proxy(): void {
        $this->setAdminUser();
        $capabilities = ['AA', 'BB'];
        $proxy =
            mod_glaaster_external::create_tool_proxy('Test proxy', $this->getExternalTestFileUrl('/test.html'), $capabilities, []);
     ...
     */
    public function test_mod_glaaster_create_tool_proxy_duplicateurl(): void {
        $this->setAdminUser();
        mod_glaaster_external::create_tool_proxy('Test proxy 1', $this->getExternalTestFileUrl('/test.html'), [], []);

        $this->expectException(moodle_exception::class);
        mod_glaaster_external::create_tool_proxy('Test proxy 2', $this->getExternalTestFileUrl('/test.html'), [], []);
    }

    /**
     * Test create_tool_proxy for a user without the required capability.
     */
    public function test_mod_glaaster_create_tool_proxy_without_capability(): void {
        $course = $this->getDataGenerator()->create_course();
     ...
     */
    public function test_mod_glaaster_delete_tool_proxy(): void {
        $this->setAdminUser();
    ...
    public function test_mod_glaaster_glaaster_get_tool_proxy_registration_request(): void {
        ...
        $request = mod_glaaster_external::glaaster_get_tool_proxy_registration_request($proxy->id);
        $request = external_api::clean_returnvalue(
            mod_glaaster_external::glaaster_get_tool_proxy_registration_request_returns(),
            $request
        );
     ...
     * Test get_tool_types.
     */
    public function test_mod_glaaster_get_tool_types(): void {
        $this->setAdminUser();
        $proxy = mod_glaaster_external::create_tool_proxy('Test proxy', $this->getExternalTestFileUrl('/test.html'), [], []);
        $proxy = (object) external_api::clean_returnvalue(mod_glaaster_external::create_tool_proxy_returns(), $proxy);
     ...
     */
    public function test_mod_glaaster_create_tool_type_nonexistant_file(): void {
        $this->expectException(moodle_exception::class);
        mod_glaaster_external::create_tool_type($this->getExternalTestFileUrl('/doesntexist.xml'), '', '');
    }

    /**
     * Test create_tool_type failure from xml that is not a cartridge.
     */
    public function test_mod_glaaster_create_tool_type_bad_file(): void {
        $this->expectException(moodle_exception::class);
        mod_glaaster_external::create_tool_type($this->getExternalTestFileUrl('/rsstest.xml'), '', '');
    }

    /**
     * Test create_tool_type as a user without the required capability.
     */
    public function test_mod_glaaster_create_tool_type_without_capability(): void {
        $course = $this->getDataGenerator()->create_course();
        $teacher = $this->getDataGenerator()->create_and_enrol($course, 'editingteacher');
        $this->setUser($teacher);
        $this->expectException(required_capability_exception::class);
    ...
    public function test_mod_glaaster_delete_tool_type_without_capability(): void {
        ...
        $this->setUser($teacher);
        $this->expectException(required_capability_exception::class);
        mod_glaaster_external::delete_tool_type($type['id']);
    }
    ...
    public function test_mod_glaaster_is_cartridge(): void {
        ...
        $result = mod_glaaster_external::is_cartridge($this->getExternalTestFileUrl('/test.html'));
        $result = external_api::clean_returnvalue(mod_glaaster_external::is_cartridge_returns(), $result);
