...
namespace mod_glaaster\local\ltiopenid;

use advanced_testcase;
use mod_glaaster\local\ltiopenid\registration_exception;
...
EOD;
     ...
     */
    private string $rminimaljson = <<<EOD
    {
        "application_type": "web",
        "response_types": ["id_token"],
        "grant_types": ["implict", "client_credentials"],
...
EOD;
     ...
     */
    private string $rminimaldljson = <<<EOD
    {
        "application_type": "web",
        "response_types": ["id_token"],
        "grant_types": ["implict", "client_credentials"],
...
EOD;
    ...
    public function test_to_config_full(): void {
        ...
        $this->assertEquals(LTI_GLAASTER_VERSION_1P3, $config->lti_ltiversion);
        ...
        $this->assertEquals(2, $config->ltiglaasterservice_gradesynchronization);
        $this->assertEquals(LTI_GLAASTER_SETTING_DELEGATE, $config->lti_acceptgrades);
        $this->assertEquals(1, $config->ltiglaasterservice_memberships);
        $this->assertEquals(0, $config->ltiglaasterservice_toolsettings);
        ...
        $this->assertEquals(LTI_GLAASTER_SETTING_ALWAYS, $config->lti_sendname);
        $this->assertEquals(LTI_GLAASTER_SETTING_ALWAYS, $config->lti_sendemailaddr);
    ...
    public function test_to_config_minimal(): void {
        $registration = json_decode($this->rminimaljson, true);
        $config = registration_helper::get()->registration_to_config($registration, 'TheClientId');
        $this->assertEquals('JWK_KEYSET', $config->lti_keytype);
        $this->assertEquals(LTI_GLAASTER_VERSION_1P3, $config->lti_ltiversion);
        $this->assertEquals('TheClientId', $config->lti_clientid);
        ...
        $this->assertEquals(0, $config->ltiglaasterservice_gradesynchronization);
        $this->assertEquals(LTI_GLAASTER_SETTING_NEVER, $config->lti_acceptgrades);
        $this->assertEquals(0, $config->ltiglaasterservice_memberships);
        $this->assertEquals(LTI_GLAASTER_SETTING_NEVER, $config->lti_sendname);
        $this->assertEquals(LTI_GLAASTER_SETTING_NEVER, $config->lti_sendemailaddr);
    ...
    public function test_to_config_minimal_with_deeplinking(): void {
        $registration = json_decode($this->rminimaldljson, true);
        $config = registration_helper::get()->registration_to_config($registration, 'TheClientId');
        $this->assertEquals(1, $config->lti_contentitem);
    ...
    public function test_config_to_registration_minimal(): void {
        $orig = json_decode($this->rminimaljson, true);
        $reghelper = registration_helper::get();
        $reg = $reghelper->config_to_registration($reghelper->registration_to_config($orig, 'clid'), 12);
        $this->assertEquals('clid', $reg['client_id']);
        $this->assertEquals($orig['response_types'], $reg['response_types']);
    ...
    public function test_config_to_registration_lti11(): void {
        ...
        $lti = $reg['https://purl.imsglobal.org/spec/lti-tool-configuration'];
        $this->assertEquals(LTI_GLAASTER_VERSION_1, $lti['version']);
    ...
    public function test_config_to_registration_lti20(): void {
        ...
        $this->assertEquals($type['name'], $reg['client_name']);
        $lti = $reg['https://purl.imsglobal.org/spec/lti-tool-configuration'];
        $this->assertEquals(LTI_GLAASTER_VERSION_2, $lti['version']);
        $this->assertEquals('LtiDeepLinkingRequest', $lti['messages'][0]['type']);
        $this->assertEquals('base.test.url', $lti['domain']);
        $this->assertEquals($type['baseurl'], $lti['target_link_uri']);
        $oauth = $lti['oauth_consumer'];
