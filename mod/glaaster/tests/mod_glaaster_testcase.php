...
/**
 * Abstract base testcase for mod_glaaster unit tests.
 *
 ...
 */
abstract class mod_glaaster_testcase extends externallib_advanced_testcase {
    /**
    ...
    protected function generate_tool_proxy(string $uniqueid): stdClass {
        // Create a tool proxy.
        $proxy = mod_glaaster_external::create_tool_proxy(
            "Test proxy $uniqueid",
            $this->getExternalTestFileUrl("/proxy$uniqueid.html"),
            [],
            []
        );
        return (object) external_api::clean_returnvalue(mod_glaaster_external::create_tool_proxy_returns(), $proxy);
    }
}
