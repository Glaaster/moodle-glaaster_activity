...
if ($err) {
    ...
    $redirect = new moodle_url('/mod/glaaster/typessettings.php', $params);
    redirect($redirect);
}

$pageurl = new moodle_url('/mod/glaaster/toolssettings.php');
if (!empty($id)) {
    $pageurl->param('id', $id);
}
if (!empty($returnto)) {
...
$form = new mod_glaaster_edit_types_form($pageurl, (object) ['isadmin' => true, 'istool' => true]);
...
$PAGE->navbar->add(
    get_string('lti_administration', 'glaaster'),
    $CFG->wwwroot . '/' . $CFG->admin . '/settings.php?section=modsettingglaaster'
);
