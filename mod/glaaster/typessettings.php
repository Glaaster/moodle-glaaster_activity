...
require_once($CFG->dirroot . '/mod/glaaster/edit_form.php');
...
if ($returnto == 'toolconfigure') {
    $returnurl = new moodle_url($CFG->wwwroot . '/mod/glaaster/toolconfigure.php');
}
...
if (!empty($id)) {
    ...
    if (!empty($type->toolproxyid)) {
        ...
        $redirect = new moodle_url('/mod/glaaster/toolssettings.php', $params);
        redirect($redirect);
    }
    $type->lti_coursecategories = '';
    if (
        $records = $DB->get_fieldset_select(
            'glaaster_types_categories',
            'categoryid',
            'typeid = :typeid',
            ['typeid' => $id]
        )
...
$pageurl = new moodle_url('/mod/glaaster/typessettings.php');
if (!empty($id)) {
    $pageurl->param('id', $id);
}
if (!empty($returnto)) {
...
$redirect = "$CFG->wwwroot/$CFG->admin/settings.php?section=modsettingglaaster&tab={$tab}";
if (!empty($returnurl)) {
    $redirect = $returnurl;
}
...
$form = new mod_glaaster_edit_types_form(
    $pageurl,
    (object) [
        'isadmin' => true,
        'istool' => false,
...
$PAGE->set_title(get_string('toolsetup', 'glaaster'));
$PAGE->set_primary_active_tab('siteadminnode');
$PAGE->set_secondary_active_tab('ltitoolconfigure');
$PAGE->navbar->add(get_string('manage_external_tools', 'glaaster'), new moodle_url('/mod/glaaster/toolconfigure.php'));
$PAGE->navbar->add(get_string('toolsetup', 'glaaster'), $PAGE->url);
$PAGE->requires->js_call_amd('mod_glaaster/coursecategory', 'init', [$type->lti_coursecategories]);

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('toolsetup', 'glaaster'));
